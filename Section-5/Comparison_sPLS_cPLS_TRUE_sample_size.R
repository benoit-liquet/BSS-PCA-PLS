#############
## Simulation section 5.3: effect of sample size
#############
...
sigma.gamma <- 1
sigma.e <- 5
sample.size.t <- c(50,100,200,500)
p <- 15
q <- 10
sparsity <- 5
...
print(jj)
set.seed(jj)
data.gen <- simu.true.model(ntrain = kk,ntest = 500,sigma.gamma,sigma.e,p,q,sparsity)
#######################################
X <- data.gen$X.train
Y <- data.gen$Y.train
...
colnames(tab.spls) <- paste("noise = ",sample.size.t)
colnames(tab.cpls) <- paste("noise = ",sample.size.t)
...
save(result,file="result_best_ss_sample_size.Rdata")
