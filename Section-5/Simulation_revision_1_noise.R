...
sigma.gamma <- 1
TABsigma.e <- c(1.5,3,6,8) #c(3,4,5,6,7)
sparsity <- 5
n <- 150
p <- 15
q <- 10
...
if(F){
cbind(spls.true,cpls.true)
cbind(colMeans(F1.spls),colMeans(F1.bss))
cbind(colMeans(sens.spls),colMeans(sens.bss))
cbind(colMeans(spe.spls),colMeans(spe.bss))
cbind(colMeans(MCC.spls),colMeans(MCC.bss))
cbind(colMeans(res.MSE.spls),colMeans(res.MSE.BSS))
res.spls <- cbind(spls.true,colMeans(res.MSE.spls),colMeans(sens.spls),colMeans(spe.spls))
res.bss <- cbind(cpls.true,colMeans(res.MSE.BSS),colMeans(sens.bss),colMeans(spe.bss))
cbind(res.bss,res.spls)
}

save(result,file=paste("result_SIMU1_NOISE",sigma.e,".Rdata",sep=""))  
}    
